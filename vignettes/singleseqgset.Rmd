...
First, we simulate data to use with splatter.
...
Next, we will add some expression data for specific genes sets to some of the clusters.
...
Now we have a count matrix of simulated data, and we know which cells are in each group via "groups". We will next read in the Hallmark Gene Sets using the GSEABase package. The Hallmark gene sets can be download directly from the Broad's website: http://software.broadinstitute.org/gsea/msigdb/index.jsp. Just make sure to choose the proper gene annotation style for your project (i.e. either gene symbols or entrez gene IDs; here we just gene symbols).

```{r}
data(h.sets.collection) #read in the collection
#Ordinarily, you would download these files locally and read them in using the getGmt function from the GSEABase package
#We have included the h.sets.collection for this Vignette
h.sets <- geneIds(h.sets.collection) #Convert from a GeneSetCollection to a list
h.sets[1:2] #Check out the first 2 gene sets
```
...
```{r}
gse.res <- wmw_gsea(expr.mat=as.matrix(ser@data),cluster.cells=logfc.data[[1]],log.fc.cluster=logfc.data[[2]],gene.sets=h.sets)
names(gse.res)
```
